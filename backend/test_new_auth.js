...
async function testAuth() {
  ...
  let res = await fetch(`${API_URL}/register`, {
    ...
    body: JSON.stringify({ full_name: 'Test Patient', email: 'patient2@example.com', password: 'password123' })
  });
  ...
  res = await fetch(`${API_URL}/login`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'patient2@example.com', password: 'password123' })
  });
  data = await res.json();
  ...
  res = await fetch(`${API_URL}/register`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ full_name: 'Test Admin', email: 'admin2@example.com', password: 'adminpassword' })
  });
  data = await res.json();
  ...
  await client.query("UPDATE users SET role='admin' WHERE email='admin2@example.com'");
  await client.end();
  ...
  res = await fetch(`${API_URL}/login`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'admin2@example.com', password: 'adminpassword' })
  });
  data = await res.json();
